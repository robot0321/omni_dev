	...
	bool prefiltered)
{
	...
	// float lat = asin(p_view.z / dist);   
	// float lon = acos(p_view.y / dist_xy) * copysign(1.0f, p_view.x);
	float lat = atan2(-p_view.y, dist_xz);   
	float lon = atan2(p_view.x, p_view.z);
	float2 p_omniuv = {(W/2-0.5)/M_PI*lon + 0.0*lat + (W/2-0.5), 0.0*lon -2*(H/2-0.5)/M_PI*lat + (H/2-0.5)}; // (u,v,1) = K@(lon,lat,1)
